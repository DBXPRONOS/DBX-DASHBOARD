...
LEAGUE_LABELS = {
    ...
    "soccer_turkey_super_league":"TUR-Super Lig","soccer_belgium_first_div":"BEL-Pro League",
    ...
    "soccer_netherlands_knvb_cup":"KNVB Cup","soccer_portugal_taca_de_portugal":"Taca de Portugal",
    ...
    "soccer_conmebol_copa_america":"Copa America","soccer_concacaf_gold_cup":"Gold Cup",
    ...
    "soccer_brazil_campeonato":"BRA-Brasileirao","soccer_brazil_serie_b":"BRA-Serie B",
    "soccer_argentina_primera_division":"ARG-Primera Division","soccer_argentina_primera_b":"ARG-Primera B",
    "soccer_chile_campeonato":"CHI-Primera Division","soccer_mexico_ligamx":"MEX-Liga MX",
    ...
    "soccer_uruguay_primera_division":"URU-Primera Division","soccer_venezuela_primera":"VEN-Primera Division",
    "soccer_paraguay_primera_division":"PAR-Primera Division","soccer_bolivia_primera_division":"BOL-Primera Division",
    "soccer_costa_rica_primera_division":"CRC-Primera Division",
    ...
    for sport in ordered_sports:
        try:
            ...
            if r.status_code != 200:
                continue
            for ev in r.json():
                if not ev.get("commence_time", "").startswith(TODAY):
                    continue
            ...
            print("Erreur " + sport + ": " + str(e))
    print("Matchs trouves: " + str(len(all_matches)))
    return all_matches
...
def process_match(m, fbref=None):
    ...
    if p1 >= 0.75 and odd_1 and odd_1 >= 1.15:
        pick_type = "safe"
        selection = home + " gagne"
        market = "1X2 - 1"
        odd_taken = odd_1
        model_prob = p1
        tags.append("Safe VIP : " + str(round(p1 * 100)) + "%")
        ...
        odd_taken = odd_over
        model_prob = po
        tags.append("Safe VIP Over 2.5 : " + str(round(po * 100)) + "%")
    elif odd_1:
        ...
        if is_val:
            ...
            odd_taken = odd_1
            model_prob = p1
            tags.append("Value +" + str(edge) + "pp edge")
    elif odd_over:
        ...
        if is_val:
            ...
            odd_taken = odd_over
            model_prob = po
            tags.append("Value Over 2.5 +" + str(edge) + "pp")
    ...
    return {
        ...
        "prob_x": round(probs["X"], 4),
        "prob_2": round(probs["2"], 4),
        "prob_over": round(probs["Over_2.5"], 4),
        "odd_1": m.get("odd_1"),
        "odd_x": m.get("odd_x"),
        "odd_2": m.get("odd_2"),
...
def main():
    ...
    total = len(active)
    score_sum = sum(p["score"] for p in active)
    score_moy = round(score_sum / total, 1) if total > 0 else 0
    output = {
        "date": TODAY,
        "updated_at": NOW,
        "stats": {
    ...
    print("OK - " + str(total) + " picks actifs / " + str(len(picks)) + " matchs analyses pour " + TODAY)
